...
Queue.prototype.state = function (state, options, fn) {
  var self = this;
  var stop, key;
  ...
  if (options && options.count) {
    stop = options.count - 1;
  } else {
    stop = -1;
  }

  key = options && options.type ? 'jobs:' + options.type + ':' + state : 'jobs:' + state;

  this.client.zrange(this.client.getKey(key), 0, stop, function(err,ids){
    var fixedIds = [];
    ids.forEach(function(id){
...
/**
 * Get jobs with state, type, and count.
 */
Queue.prototype.getJobs = function (state, type, count, fn) {
  var options;
  ...
  return this.state(state, options, fn);
};
